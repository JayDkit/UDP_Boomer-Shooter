...
namespace GDApp.Content.Scripts.Level
{
    public class Level1
    {
        ...
        public void InitializeFloors(Scene level)
        {
            ...
            var material = new BasicMaterial("simple diffuse", new BasicShader(Application.Content), textureDictionary["floor"]);

            var archetypalQuad = new GameObject("floor", GameObjectType.Skybox);
            var renderer = new MeshRenderer(new QuadMesh(), material);
        ...
        public void InitializeWalls(Scene level)
        {
            ...
            var material = new BasicMaterial("simple diffuse", new BasicShader(Application.Content), textureDictionary["brick"]);

            var archetypalWall = new GameObject("wall", GameObjectType.Architecture);
            var renderer = new MeshRenderer(new CubeMesh(), material);
        ...
        public void InitializePickups(Scene level)
        {
            ...
            //for CA2.
            var speedMaterial = new BasicMaterial("model material", new BasicShader(Application.Content), Content.Load<Texture2D>("Assets/Textures/Pickups/ElectricityTexture"));
            ...
            var speedRenderer = new ModelRenderer(Content.Load<Model>("Assets/Models/Pickups/SpeedPickup"), speedMaterial);
            ...
            var healthMaterial = new BasicMaterial("model material", new BasicShader(Application.Content), Content.Load<Texture2D>("Assets/Textures/Pickups/HealthKitTexture"));

            var healthPickup = new GameObject("health_pickup", GameObjectType.Consumable);
            var healthRenderer = new ModelRenderer(Content.Load<Model>("Assets/Models/Pickups/HealthKit"), healthMaterial);
        ...
        {
            var turretMaterial = new BasicMaterial("model material", new BasicShader(Application.Content), Content.Load<Texture2D>("Assets/Demo/Textures/grey"));

            var turret = new GameObject("turret", GameObjectType.NPC);
            var turretRenderer = new ModelRenderer(Content.Load<Model>("Assets/Models/Turret"), turretMaterial);
